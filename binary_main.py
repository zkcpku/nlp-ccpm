...
from model.binary_cls import BinaryRobertaChoice

from dataload.binary_dataset import DataBinaryCls, get_preprocess_function
...
class ModelArguments:
    ...
    load_model_path: str = field(
        default=None,
        metadata={"help": "Path to the model to load (optional)"},
    )
...
def main():
    ...
    if model_args.load_model_path is not None:
        model.load_state_dict(torch.load(model_args.load_model_path))
        logger.warning("load model from " + model_args.load_model_path)
    ...
    preprocess_function = get_preprocess_function(max_seq_length, tokenizer, data_args.pad_to_max_length)
    ...
    # Data collator
    data_collator = DataBinaryCls(tokenizer=tokenizer, pad_to_multiple_of=8 if training_args.fp16 else None)
    ...
    if training_args.do_eval:
        ...
        metrics = trainer.evaluate(ignore_keys = ['hidden_states','attentions','input_ids'])
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        test_result = trainer.predict(test_dataset,ignore_keys = ['hidden_states','attentions','input_ids'])
        import pickle
        with open(training_args.output_dir + 'test_result.pkl','wb') as f:
            pickle.dump({'test_result':test_result,'test_dataset':test_dataset},f)
