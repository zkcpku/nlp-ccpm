...
from model.binary_ner import BinaryNerRoberta

from dataload.binary_nerSingle_dataset import get_preprocess_function
from dataload.binary_nerSingle_dataset import DataBinaryNerSingleCls
...
def main():
    ...
    # Data collator
    data_collator = DataBinaryNerSingleCls(tokenizer=tokenizer, pad_to_multiple_of=8 if training_args.fp16 else None)
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        test_result = trainer.predict(test_dataset,ignore_keys = ['hidden_states','attentions'])
        import pickle
